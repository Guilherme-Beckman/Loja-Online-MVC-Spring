...
public List<CartItem>  getAllProducts(Long id){
	if (id !=null && id != 0) {
		...
		List<CartItem> allProducts = cart.getProductcs();
...
public Cart addProductToCart(Long id, Long productId){
	if (id !=null && id != 0 && productId !=null && productId != 0) {
			...
			CartItem cartItem = cartItemService.insert(product);
			List<CartItem> allProducts = cart.getProductcs();
...
@Transactional
public Cart deleteProductInCart(Long id, Long cartItemID){
	if (id !=null && id != 0 && cartItemID !=null && cartItemID != 0) {
		Cart cart = repository.findById(id).orElseThrow(CartNotFoundException::new);
		CartItem product = cartItemService.findById(cartItemID);
		List<CartItem> allProducts = cart.getProductcs();
